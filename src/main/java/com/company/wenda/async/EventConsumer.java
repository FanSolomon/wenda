...
public class EventConsumer implements InitializingBean, ApplicationContextAware {
    ...
    public void afterPropertiesSet() throws Exception {
        //找出所有EventHandler接口具体的实现类
        Map<String, EventHandler> beans = applicationContext.getBeansOfType(EventHandler.class);
        if (beans != null) {
            for (Map.Entry<String, EventHandler> entry : beans.entrySet()) {
                List<EventType> eventTypes = entry.getValue().getSupportEventTypes();   //给这个EventHandler加上它关注的事件

                //在这个事件处理的函数中把自己注册上
                for (EventType type : eventTypes) {
                    if (!config.containsKey(type)) {    //第一次注册这个事件
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                while (true) {
                    ...
                    //找这个Event关联的各种Handler进行处理
                    for (String message : events) {
                        if (message.equals(key)) {  //过滤掉第一个返回值key
                            continue;
                        }
